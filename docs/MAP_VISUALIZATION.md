...
### Frontend (React)
*   **Libraries:** `react-leaflet` for map rendering, `@turf/turf` for spatial analysis, and `leaflet.heat` for the 30-day hotspot overlay.
*   **OpenStreetMap GeoJSON Integration:** The real-world curves and geometry of NH-275 are extracted using OpenStreetMap (OSM) via Overpass API into a `nh275-osm.geojson` file.
*   **Turf.js Spatial Analysis:** Turf.js is used to analyze the OSM GeoJSON to:
    - Split the highway polyline into 11 distinct `Feature` objects matching `zone_id`s based on exact lengths.
    - Dynamically snap vehicles to the correct path using nearest-point-on-line calculations.
    - Calculate distances between moving coordinates and checkpoint features.
