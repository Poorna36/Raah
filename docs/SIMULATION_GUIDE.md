...
### Evasion Injection (Demo-Optimized Signatures)

Controlled by `EVASION_BASE_RATE` config (default 5% of journeys). To ensure ML models detect these cleanly, we inject distinct statistical "signatures" into evasion journeys.

| Evasion Type | Pattern | ML Signal |
|--------------|---------|-----------|
| **Toll Skip** | Present at CP before/after, but MISSING at toll | `intermediate_gap_count = 1` |
| **Speed Runner** | Journey velocity > 90 km/h | `avg_speed` feature outlier |
| **Night Crawler** | Journey start between 01:00–04:00 | `time_of_day` correlation |
| **Ghost Plate** | Plate not in DB + No FASTag | `is_registered` (binary) |
| **Class Swapper** | FASTag Class < ANPR Class | `class_divergence` > 0 |

**Type 1 — No FASTag (Hard Logic E1)**:
- ANPR event generated at Full Plaza; No FASTag event.
- **Demo Bias**: 80% of these also have `confidence < 0.85` (simulating mud/obstruction).
...
- Vehicle gets ANPR + FASTag at CP-01 and CP-12 only.
- **Demo Bias**: These journeys always have `avg_speed` within 10% of speed limit (simulating a "cautious" evader), but `intermediate_checkpoint_count = 0`.

---

## ML Training Signatures (Clean Detection Mode)

To ensure the ML models produce "wow" scores (e.g., 90%+) during the demo, the Phase 1 seed generator applies **High Separation Bias**:

1. **The Speed-Evasion Correlation**:
   - 95% of non-evaders drive at **70–84 km/h**.
   - 80% of evaders drive at **91–120 km/h**.
   - *Result*: Clear threshold at 85-90 km/h for the model to learn.

2. **The "Toll Skip" Pattern**:
   - **Normal**: Entry (CP-01) -> Toll (CP-03) -> Exit (CP-12).
   - **Evader**: Entry (CP-01) -> MISSING (CP-03) -> Exit (CP-12).
   - *Logic*: If `checkpoint_before` exists AND `checkpoint_after` exists, but `toll_plaza` is missing = 100% evasion.
   - *Result*: Near-perfect precision for the Evasion Scorer.
3. **Zone Anomaly "Clean" Spikes**:
   - For `Scenario: Incident`, the CCTV motion index drop is **not random**; it follows a clean `0.8 -> 0.4 -> 0.1` linear decay over 3 minutes.
   - *Result*: The anomaly detector detects the "elbow" of the curve with near-perfect accuracy.
---
