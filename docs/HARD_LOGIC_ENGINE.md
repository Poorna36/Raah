...
Journey Update Received
    ...
    → E6: Check Toll Plaza Skip (bypass detection)
...
Speed limits by class (per demo requirements and highway regulations):

| Class | Normal Speed (<85) | Evader Trigger (>90) | Physical Limit (E5) |
|-------|-------------------|----------------------|----------------------|
| Car, LMV | 70–84 km/h | 91+ km/h | 120 km/h |
| LCV | 65–84 km/h | 91+ km/h | 100 km/h |
| Bus | 60–84 km/h | 91+ km/h | 90 km/h |
| Truck, MAV | 55–84 km/h | 91+ km/h | 80 km/h |
...
### Rule E6: Toll Plaza Skip (Bypass Detection)

**Legal basis**: NH Fee Rules 2008, **Rule 5** — obligation to pay fee. Using bypass roads to deliberately avoid a toll station while remaining on the highway corridor constitutes evasion.

**Trigger condition**:
```
Vehicle detected at Checkpoint A (e.g., CP-02)
AND
Vehicle detected at Checkpoint B (e.g., CP-04)
AND
No ANPR or FASTag event recorded at Intermediate Toll Plaza (e.g., CP-03)
AND
Vehicle is NOT exempt (Rule EX)
```

**Input**: Journey state update showing gap in sequential checkpoints where a toll plaza is located.

**Output**:
```json
{
  "rule_code": "E6",
  "rule_description": "Toll Plaza Skip detected — vehicle bypassed mandatory toll station",
  "legal_reference": "NH Fee Rules 2008, Rule 5 & 15",
  "violation_type": "toll_plaza_bypass",
  "plate": "KA09AB1234",
  "checkpoint_id": "CP-03 (Skipped)",
  "evidence_ids": ["cp_before_uuid", "cp_after_uuid"],
  "confidence": 1.0,
  "missed_amount": 80.0
}
```
