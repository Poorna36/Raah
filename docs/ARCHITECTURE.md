...
| `commuter-portal` | 5174 | React + Vite | Mobile web — commuter interface |
...
└───────────┬───────────┴────────────┬─────────────┘
            ...
            └──────► Alert Engine (backend) ◄───────┘
              ...
              ┌─────────┴──────────┐
              ▼                    ▼
   Authority Dashboard     Commuter Portal
...
| `/commuter` | Commuter Portal | `route_alert`, `journey_progress`, `zone_warning` | `track_journey`, `query_route` |
...
postgres → redis → backend → simulator → ml_service → authority-dashboard → commuter-portal
```
...
# Terminal 6: Commuter Portal
cd commuter-portal && npm install && npm run dev -- --port 5174
```
